...
import { loadEmbeddings, appendLongMemory, readLongMemory, recallMemory } from './memory';
...
export class SapperPanel implements vscode.WebviewViewProvider {
    ...
    webviewView.webview.onDidReceiveMessage(async (msg) => {
      switch (msg.type) {
        ...
        case 'searchWorkspaceFiles':
          ...
          break;
        case 'setMode':
          this._currentMode = msg.mode as 'ask' | 'edit' | 'agent';
          if (this._core) { this._core.updateOptions({ mode: this._currentMode }); }
        ...
        case 'undoLast':
          { const removed = this._core?.popLastTurn() ?? false;
            ...
            if (removed) { this._sendContextInfo(); } }
          break;
        case 'getDiagnostics':
        ...
        case 'shellApprovalResponse':
          { const resolver = this._shellApprovalResolvers.get(msg.id);
            if (resolver) { this._shellApprovalResolvers.delete(msg.id); resolver(!!msg.approved); } }
          break;
        case 'listSessions':
          this._listSessions();
          break;
        ...
        case 'loadSession':
          ...
          break;
        case 'deleteSession':
          SapperCore.deleteSession(this._getSapperDir(), msg.name);
          this._listSessions();
          break;
      }
    });
  ...
  private _shellApprovalResolvers = new Map<number, (approved: boolean) => void>();
  private _shellApprovalCounter = 0;
  private _currentMode: 'ask' | 'edit' | 'agent' = 'agent';
  ...
  private _ensureCore(model: string): void {
    ...
    this._core = new SapperCore({
      ...
      disabledTools: ws.get<string[]>('disabledTools') ?? [],
      systemPromptOverride: ws.get<string>('systemPrompt') || '',
      maxContextTokens: ws.get<number>('maxContextTokens') ?? 0,
      mode: this._currentMode,
      onChunk: (text) => {
        if (!this._streaming) { this._streaming = true; this._post({ type: 'streamStart' }); }
        this._post({ type: 'streamChunk', text });
      },
      onStatus: (status) => {
        // Map core status strings to UI state + text
        const map: Record<string, { state: string; text: string }> = {
        ...
        if (status === 'done' || status === 'idle' || status === 'error') {
          if (this._streaming) { this._streaming = false; this._post({ type: 'streamEnd' }); }
        }
      },
      onToolStart: (tool, p) => this._post({ type: 'tool', tool, path: p, status: 'running' }),
      onToolEnd: (tool, p, success) =>
        this._post({ type: 'tool', tool, path: p, status: success ? 'success' : 'error' }),
      onFileChange: (action, p) => this._post({ type: 'fileChange', action, path: p }),
      onShellApprovalRequired: (cmd: string) => {
  ...
  private async _onReady(): Promise<void> {
    ...
    const logoUri = this._view!.webview.asWebviewUri(
      vscode.Uri.joinPath(this._context.extensionUri, 'media', 'sapper-logo.svg')
    ).toString();
    this._post({
      type: 'init',
  ...
  private async _onChat(text: string, attachedContext?: string): Promise<void> {
  ...
  private _sendAgentsAndSkills(): void {
  ...
  private _sendSettings(): void {
    ...
    this._post({
      ...
      settings: {
        ollamaHost: ws.get<string>('ollamaHost') || 'http://127.0.0.1:11434',
        defaultModel: ws.get<string>('defaultModel') || '',
        toolRoundLimit: ws.get<number>('toolRoundLimit') ?? 40,
        autoAttach: ws.get<boolean>('autoAttach') ?? true,
        shellEnabled: ws.get<boolean>('shellEnabled') ?? true,
        systemPrompt: ws.get<string>('systemPrompt') || '',
        disabledTools: ws.get<string[]>('disabledTools') ?? [],
        maxContextTokens: ws.get<number>('maxContextTokens') ?? 0,
        contextLimit: ws.get<number>('contextLimit') ?? 0,
        summarizeTriggerPercent: ws.get<number>('summarizeTriggerPercent') ?? 65,
      },
    });
  }
  ...
  private async _saveSettings(settings: Record<string, any>): Promise<void> {
    ...
    for (const [key, value] of Object.entries(settings)) {
      try { await ws.update(key, value, target); } catch (_) { /* ignore */ }
    }
    this._post({ type: 'settingsSaved' });
    // Re-create core with new settings
    if (this._currentModel) { this._ensureCore(this._currentModel); }
  }

  private _sendWorkspaceTree(relPath: string): void {
    const workingDir = this._getWorkingDir();
    const target = relPath ? path.join(workingDir, relPath) : workingDir;
    // Block path traversal
    if (!target.startsWith(workingDir)) { return; }
    try {
  ...
  private async _handleDroppedFiles(uris: string[]): Promise<void> {
    const fsPaths = uris.map(u => {
      try { return vscode.Uri.parse(u).fsPath; } catch { return u; }
    }).filter(Boolean);
    const files = await this._readFilesForAttach(fsPaths);
    this._post({ type: 'filePicked', files });
  }

  private async _readFilesForAttach(fsPaths: string[]): Promise<Array<{name: string; path: string; content: string; isDir: boolean}>> {
    const fs = require('fs') as typeof import('fs');
    const MAX = 50000;
    const workingDir = this._getWorkingDir();
    const result: Array<{name: string; path: string; content: string; isDir: boolean}> = [];
    for (const fp of fsPaths) {
      try {
        ...
        if (stat.isDirectory()) {
          ...
          const listing = (entries as any[]).map((e: any) =>
            `  ${e.isDirectory() ? '[dir] ' : '[file]'} ${e.name}`).join('\n');
  ...
  private _sendContextInfo(): void {
    ...
    const contextLimit = ws.get<number>('maxContextTokens') ?? 0;
    const summarizeTriggerPercent = ws.get<number>('summarizeTriggerPercent') ?? 65;
    const toolRoundLimit = ws.get<number>('toolRoundLimit') ?? 40;
    const systemPrompt = ws.get<string>('systemPrompt') || '';
    this._post({
      type: 'contextInfo',
      ...stats,
      contextLimit,
  ...
  private _postWorkspaceIndex(): void {
    try {
      const idx = SapperCore.buildWorkspaceIndex(this._getWorkingDir());
  ...
  private _sendMemoryInfo(): void {
    const sapperDir = this._getSapperDir();
    try {
      const chunks = loadEmbeddings(sapperDir);
      const notes = readLongMemory(sapperDir);
  ...
  private _searchMemory(query: string): void {
    const sapperDir = this._getSapperDir();
    try {
      const result = recallMemory(sapperDir, query, 5);
  ...
  private _saveNote(text: string): void {
    if (!text.trim()) { return; }
    const sapperDir = this._getSapperDir();
    try {
      appendLongMemory(sapperDir, text);
      this._sendMemoryInfo();
  ...
  private _saveCodeBlock(relPath: string, content: string): void {
    const fs = require('fs') as typeof import('fs');
    const workingDir = this._getWorkingDir();
  ...
  private async _applyDiff(relPath: string, newContent: string): Promise<void> {
    const fs = require('fs') as typeof import('fs');
    const workingDir = this._getWorkingDir();
    const absPath = relPath.startsWith('/') ? relPath : path.join(workingDir, relPath);
    try {
  ...
  private _searchWorkspaceFiles(query: string): void {
    const workingDir = this._getWorkingDir();
    const fs = require('fs') as typeof import('fs');
  ...
  private _sendDiagnostics(): void {
    ...
    const items: Array<{ file: string; severity: string; message: string; line: number }> = [];
  ...
  private _listSessions(): void {
    const sessions = SapperCore.listSessions(this._getSapperDir());
    this._post({ type: 'sessionList', sessions });
  }

  private _saveSession(name: string): void {
    if (!name || !this._core) { return; }
  ...
  private _loadSession(name: string): void {
  ...
  private _sendSystemPromptPreview(): void {
    ...
    const ws = vscode.workspace.getConfiguration('sapper');
    const custom = ws.get<string>('systemPrompt') || '';
